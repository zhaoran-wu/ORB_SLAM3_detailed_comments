...
bool LoopClosing::NewDetectCommonRegions()
{
    ...
    // 3.其他情况(单目,rgbd)且当前地图关键帧少于12则不进行检测
    ...
    // Step 1 基于前一帧的历史信息判断是否进行时序几何校验, 注意这里是基于共视几何校验失败才会运行的代码,阅读代码的时候可以先看后面

    // Loop candidates
    bool bLoopDetectedInKF = false;
    bool bCheckSpatial = false;
    // Step 1.1 回环的时序几何校验: 这里的判断条件里mnLoopNumCoincidences刚开始为0, 所以可以先跳过看后面
    // 如果回环的共视几何验证成功帧数目大于0
    ...
    // Step 1.2 融合的时序几何校验: 这里的判断条件里mnMergeNumCoincidences刚开始为0, 所以可以先跳过看后面
    // 如果融合的共视几何验证成功帧数目大于0
    ...
    // Step 1.3 若校验成功则把当前帧添加进数据库,且返回true表示找到共同区域
    ...
    // Step 2 若当前关键帧没有被检测到回环或融合,则分别通过bow拿到当前帧最好的三个回环候选帧和融合候选帧
